...
- [ ] **REG-01**: Registry expansion — 5 new entries with meta.layer/meta.pattern + meta.heavy:true on P3 lazy entries (4/5 shipped: SFDataTable + SFCombobox + SFRichEditor + SFFileUpload + SFDateRangePicker; registry now 57 items)
- [ ] **BND-08**: `/` First Load JS remains under 200 KB hard target after all 5 components ship (current: 192 KB / 200 KB after Phase 75 close — 8 KB headroom into Phase 76 final-gate)
...
*Last updated: 2026-05-02 — Phase 75 (SFDateRangePicker) closed. 7/7 reqs satisfied (DR-01..DR-06, TST-03). Pattern C #5: SFCalendarLazy (P3 lazy boundary, inherited Phase 41) + SFPopover trio + SFInput (read-only trigger via `asChild`) + SFButton presets-rail; controlled-only `value: DateRange | undefined` API; `import type { Locale } from 'date-fns/locale'` (date-fns absent from runtime — Pattern C #5 zero-new-runtime-deps); SSR-safe `new Date()` ONLY inside `useMemo` (defaultMonth) and consumer-supplied preset `getValue` thunks. Homepage First Load JS = 192 KB / 200 KB (registry 56 → 57, +4.4 KB delta from `cmdk` no-op + Phase-74 carry-forward; 8 KB headroom into Phase 76). RDP v9 modifier assertion idiom: spec targets `[data-range-start="true"]` / `[data-range-middle="true"]` / `[data-range-end="true"]` data-attrs (RDP-emitted, NOT authored — classNames map is value-only). 10/10 specs PASS (7 hydration/acceptance + 3 axe-core WCAG AA). 4 deferred manual UAT items in 75-HUMAN-UAT.md (M-01 iPhone touch-target, M-02 NVDA/VoiceOver announce, M-03 375px popover collision, M-04 cross-browser type="time" keyboard parity). Wave-2 spec-bug pattern documented: agent shipped tests that compiled but never ran (`m.text()` called as method on string property, fixture-scoped selectors hitting Radix-portaled content, `[class*="range_start"]` expecting RDP to emit literal class tokens, `dialog-name` axe rule misnamed); orchestrator repaired inline at `041a86e` after live re-run. Phase 76 (Final Gate — Chromatic + bundle audit + docs) is next.*
