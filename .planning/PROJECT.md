...
*Last updated: 2026-05-02 — Phase 74 (SFFileUpload) closed. 7/7 reqs satisfied (FU-01..05, TST-03 SFFileUpload-scope, TST-04). Pattern C zero-dep precedent extended again: native `File`/`DataTransfer`/`onPaste` + hidden `<input type="file">` keyboard fallback + `URL.createObjectURL` paired with `revokeObjectURL` in `useEffect` cleanup (no `FileReader.readAsDataURL` — memory-safe for large files). Homepage First Load JS held at 187.6 KB / 200 KB (registry items 54 → 55, sf-file-upload tree-shake-clean from `/page` chunk). 18/18 specs (12 Playwright via `locator.setInputFiles()` + 6 axe-core WCAG AA) + 8 Storybook stories incl. `DragActive` play()+`fireEvent.dragOver` from `storybook/test` (v10 path). `dataTransfer.files` Chromium gap documented as FU-05/TST-04 first-class deliverable with 3 primary-source citations. 2 pre-existing bugs auto-fixed during execution: sf-progress.tsx aria-valuenow forwarding + error chip contrast (3.76:1 → AA-pass). Code review: 0 critical / 2 warning (axe `aria-progressbar-name` rule may not exist; redundant per-story chromatic.delay) / 4 info advisory. 7 manual sign-off items M-01..M-07 in 74-HUMAN-UAT.md (real-Chromium drag/drop, blob revoke audit, VoiceOver speech, theme parity, R-64-d Space-key belt-and-suspenders). Phase 75 (SFDateRangePicker, react-day-picker already in stack) is next.*
