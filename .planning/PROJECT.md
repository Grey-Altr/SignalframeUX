...
| `_dep_X_decision` ratification block (REQ-ID-namespaced extension of `_wmk_01_decision`) for runtime npm deps | v1.10 (Phase 71+73) | ✓ Good — DEP-01 (`_dep_dt_01_decision` for @tanstack/react-table@8.21.3) + DEP-02 (`_dep_re_01_decision` for @tiptap/{react,pm,starter-kit,extension-link}@3.22.5) authored at plan-time with measured (not estimated) bundle_evidence; both blocks live in source + ratified to main |
| Pattern B P3 lazy posture (next/dynamic ssr:false; NEVER in barrel; NEVER in optimizePackageImports) for heavy deps | v1.10 (Phase 71+73) | ✓ Good — sf-data-table-lazy + sf-rich-editor-lazy keep TanStack Table + Tiptap entirely out of homepage First Load chunks; verified by chunk-manifest absence audit at Phase 76 BND-08 (zero offending chunks across 12 homepage chunks; bundle 188.1 KB / 200 KB with 11.9 KB headroom) |
| Pattern B registry-deferral consolidation at final-gate phase (NOT same-commit per-phase) | v1.10 (Phase 76) | ✓ Good — Phases 71/73 intentionally shipped without registry artifacts because Pattern B + `meta.heavy: true` is the v1.10 final-gate consolidation flag; REG-01 closed in Phase 76 same-commit cohort `58aa842` (registry items[] 56→58 + 2 standalones with inline content + SCAFFOLDING count v1.3 49 → v1.10 58) |
| AES-05 final-gate user-checkpoint with mechanical-evidence corroboration | v1.10 (Phase 76) | ✓ Good — 5-item Chromatic audit (zero rounded corners + no react-day-picker default blue + no Tiptap system fonts + blessed-stop spacing + OKLCH-only colors) all backed by per-phase grep evidence already PASS in 5 VERIFICATIONs; user `continue` resume-signal interpreted as `approved` per checkpoint contract; recovery path via `/pde:plan-phase 76 --gaps` documented if Chromatic regression surfaces post-close |
