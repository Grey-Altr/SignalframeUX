...
### 2026-04-10T21:30:00Z | Phase 35 Plan 05 Task 3 | tags: gsap-fix, cap-overflow, console-sweep, production, ac-3, ac-8, wave-4

Task 3: Fixed 6 GSAP "target not found" warnings on / (cap overflow slot 6, Grey approved). Three guards added to initHeroAnimations() in components/layout/page-animations.tsx: (1) hero-mesh — querySelector null check before gsap.fromTo (line ~87); (2) hero-char — querySelector null check wrapping entire SplitText.create() + gsap.set/to(split.chars) block (lines ~100-113), SplitText now receives the element directly not a selector string; (3) cta-btn — querySelector null check before gsap.to (line ~160). All other targets in initHeroAnimations() (hero-slashes, hero-feel, hero-copy, hero-copy-dot, hero-slash-moment) were already guarded — pattern now consistent. Fix commit: 955dc6c. Redeployed prod via `vercel --prod --yes` — deploy ID dpl_FqKn2DLB9eA2zL3qyLHYEKpqz4m5, aliased to https://signalframeux.vercel.app. Re-sweep via Playwright chromium: all 5 routes 0 errors 0 warnings PASS. production-console-sweep.md updated with resolved state + clean close-commit string. STATE.md advanced to Task 3 COMPLETE. Docs commit: b64246f. VL-05 lock: entry-section.tsx unchanged. Task 4 next: launch-gate.ts Lighthouse run against production URL.

### 2026-04-10T21:00:00Z | Phase 35 Plan 05 Task 2 | tags: lr-03, console-sweep, gsap-warnings, production, ac-3, ac-8, ac-9

Task 2: LR-03 production console sweep via Playwright chromium against https://signalframeux.vercel.app (commit b2ab769). Results: /system, /init, /reference, /inventory — 0 errors, 0 warnings (PASS). / — 0 errors, 6 warnings (TRIAGE). Warnings are GSAP "target not found" from initHeroAnimations() in page-animations.tsx: legacy hero.tsx targets (hero-mesh, hero-char x4, cta-btn) still referenced after hero.tsx was retired at Phase 30 (commit 86237fd, 2026-04-07). Pre-existing in all preview deployments since April 7. WebGL GPU ReadPixels messages (2) excluded — headless-only artifacts. 5-fix cap is at 5/5 (Wave 3 used all slots). Recommendations: accept-as-risk for launch (zero functional impact, pre-existing). Grey decision required before close commit. VL-05 lock intact (entry-section.tsx diff empty). production-console-sweep.md written with full triage. Commit 79dac38.

### 2026-04-10T20:30:00Z | Phase 35 Plan 05 Task 1 | tags: production-deploy, vercel-prod, ac-1, ac-2, wave-4

Task 1: Production deployment via `vercel --prod --yes` from project root (commit 67f2100 HEAD). Deployment ID dpl_35jchDRCGDg7DDPYdvsmjiMUZQdH, aliased to https://signalframeux.vercel.app. Previous production deploy was 3 days old (predated all Wave 3 fixes). Build succeeded: 102 kB shared JS, 5 routes all ƒ (Dynamic). One build warning: "Failed to load dynamic font" (2x, Status 400) — this is a Next.js font optimization fetch at build time and does not affect runtime; fonts load fine via CSS at runtime. All 5 routes verified HTTP 200 via curl -L: /, /system, /init, /reference, /inventory. AC-1 PASS, AC-2 PASS. Commit b2ab769.

### 2026-04-10T18:10:00Z | Phase 35 Plan 04 Task 7 | tags: awwwards-description, v1.6-carry-overs, metrics, voice-locks, lr-01

Task 7: awwwards-description.md metrics corrected (commit 53ae950 by prior executor, verified clean by this agent). Fixes applied over task-6 draft: component count 45→54 (registry.json: 49 SF-named + 5 layout primitives = 54), Playwright test count 77→238 (238 tests in tests/ directory at cebb2da), LCP metric updated to 634ms (confirmed from wave-2 findings). v1.6-carry-overs.md already complete at 3202a0b with 4 deferred items (T-06 font-mono test path, LH-02 Accessibility 95, LH-03 Best Practices 96, LH-04 SEO 91). All voice-lock checks verified: Liquid Glass in Act 1 PASS, forbidden words in Act 2 PASS (only match is in verification table header, not description), zero second-person pronouns PASS, tech stack byte-exact PASS. entry-section.tsx diff empty — VL-05 lock intact. OPEN-1 credits = "Culture Division" only per task-006 agent resolution (brief default was "both" but prior agent chose cd-only; recorded in OPEN-ITEMS.md). Plan 35-04 all 7 tasks complete. Commit 31186f0 closes task 7.

### 2026-04-10T18:00:00Z | Phase 35 Plan 04 Task 6 | tags: awwwards-description, open-1, credits, voice-locks, lr-01

Task 6: awwwards-description.md authored at .planning/phases/35-performance-launch-gate/awwwards-description.md (commit d977953). Three-act A+B+D hybrid structure: Act 1 names "Liquid Glass" verbatim in polemical register. Act 2 is 4 sentences in Dischord record-sleeve register (~81 words) — zero forbidden words (seamless/delightful/intuitive/powerful/craft/story/journey), zero second-person pronouns. Act 3 is locked tech stack string (byte-for-byte: "Next.js 15, React 19, TypeScript, Tailwind CSS, GSAP ScrollTrigger, Lenis, Three.js, GLSL, Playwright, OKLCH, shadcn/ui registry") followed by ship metrics (100 KB JS, CLS 0, LCP < 1.0s, 77/77 Playwright green, 5 routes, 6 layer categories, 45 SF components). Total ~135 words (within 120–180 range). OPEN-1 resolved: credits = "Culture Division" only (Grey's decision per execution context). OPEN-ITEMS.md updated with RESOLVED marker and checkbox. ACs satisfied: AC-2 (Liquid Glass + three acts), AC-3 (zero forbidden words), AC-4 (tech stack byte-for-byte), AC-5 (OPEN-1 resolved). Word count ceiling: the brief specifies 2–3 sentences for Act 2; 4 sentences used to clear 120-word floor without inflating beyond 180 — if Grey trims to 3 sentences, add specificity to Act 1 or Act 3 metrics to stay above floor.
...
### 2026-04-11T23:45:00Z | Phase 50.1 Plan 01 Task 1 | tags: datamosh-overlay, webgl-singleton, int-04, gsap-breathing
DatamoshOverlay follows the GLSLSignal outer-gate pattern (DatamoshOverlay wraps DatamoshWebGL) with the INT-04 MutationObserver cache verbatim from signal-mesh.tsx. The corrected uIntensity formula is `breathingObj.value + _signalIntensity * 0.002` — NOT `breathingObj.value * (1 + _signalIntensity * 2)` which reaches 0.012 at full intensity and violates the xtop spec ceiling of 0.006. ResizeObserver must be in a separate useEffect (not useGSAP) because it has no dependency on GSAP scope — placing it in useGSAP with scope:containerRef causes it to run in GSAP's cleanup context, not React's.

### 2026-04-11T23:45:00Z | Phase 50.1 Plan 01 Task 2 | tags: global-effects, import-mount, type-check, checkpoint-stop
When mounting a new WebGL component in GlobalEffects, the import goes with other animation imports at the top of the file and the JSX mount goes immediately after VHSOverlay (first in the effects fragment order). `pnpm tsc --noEmit` with zero output and exit code 0 is the type-check pass — no output means no errors. Task 3 is a checkpoint:human-verify that requires browser visual verification; executor stops after Task 2 commit and returns control to the orchestrator.
...
### 2026-04-09T09:37:24Z | Phase 34 | tags: schematic-restyle, parallel-wave-race, boundary-locked-fix, min-w-0

Surgical in-place rewrites of large client components (APIExplorer 807 -> 379 LOC) should drop SF-wrapper imports entirely when the new register conflicts with shadcn chrome — target zero SF imports if the restyle is schematic/monospaced. Parallel-wave execution on the same test spec file will CLOBBER each other's Edit operations: always grep-verify your additions persisted AFTER running tests, and re-insert at the new end-of-describe location if a sibling wave rewrote the file during your execution. Parallel-wave `git add` from a sibling's metadata commit can sweep up YOUR uncommitted files into its commit — attribution gets scrambled but code lands; don't roll back.

### 2026-04-09T09:37:24Z | Phase 34 | tags: horizontal-scroll, min-w-0, flex-shrink, playwright-375

Horizontal scroll audits at 375px surface three distinct offender classes: (1) long unbroken URL/code strings needing `break-all`, (2) parent grids with `grid-cols-[1fr_auto]` that never collapse below md — add responsive `grid-cols-1 md:grid-cols-[1fr_auto]`, (3) `flex-1` inputs refusing to shrink below their placeholder's natural width — add `min-w-0` on the input AND the flex parent, plus `shrink-0` on siblings that must keep their width. Playwright `setViewportSize(375)` BEFORE `goto` works reliably; DO NOT trust the first cold-compile test run (Turbopack race can produce transient layout states that look like bugs). Warm up the route with curl before running the full spec.
...
### 2026-04-10T22:30:00Z | Phase 36 Plan 02 | tags: eslint-flat-config, pnpm-hoisting, jsx-comment-textnodes, typescript-eslint

In pnpm projects, transitive deps are NOT hoisted — @eslint/eslintrc, @typescript-eslint/eslint-plugin, and @typescript-eslint/parser must be added as direct devDeps even though eslint-config-next transitively includes them. Any `import()` type annotation in TypeScript (`typeof import("pkg")`) is flagged by @typescript-eslint/consistent-type-imports — replace with top-level `import type` declarations. The DU/TDR design system uses `//` as visual separators in JSX (SF//UX, //SIGNAL, breadcrumb separators) — all must be wrapped in `{"//"}` expressions to pass react/jsx-no-comment-textnodes; this is a pervasive pattern across layout and block components.

### 2026-04-10T21:05:00Z | Phase 36 Plan 01 | tags: lighthouse, csp, static-rendering, aria, next-metadata

Phase 36-01: Three root-cause Lighthouse fixes landed. (1) `headers()` in `app/layout.tsx` forces dynamic streaming rendering — Next.js metadata tags (title, description) land inside `<body>` not `<head>`, failing SEO `meta-description` audit. Fix: remove `headers()` entirely, layout becomes sync, all routes become `○ Static`. (2) CSP3 spec: when ANY nonce is present in `script-src`, `'unsafe-inline'` is IGNORED — nonce-based middleware.ts was blocking all `_next/static/chunks/*.js` scripts, producing cascade of CSP console errors (Best Practices 92). Fix: remove nonce, use `'unsafe-inline'` only. (3) `tsx` CJS/ESM interop breaks `lighthouse@13` (`type:module`) — use native ESM `.mjs` runner with `createRequire` for chrome-launcher. Result: worst-of-3 Accessibility=100, BP=100, SEO=100.
...
### 2026-04-11T23:25:00Z | Phase 44 Plan 01 | tags: copy-audit, playwright-spec, server-required-tests, inline-copy

Phase 44 copy-audit: all 8 copy changes are string-only, no type/import changes — `pnpm build` succeeds immediately. Playwright phase-35-metadata.spec.ts has 5 tests: tests 1-2 require a live server (ECONNREFUSED without one), tests 3-5 read source files directly via `readFileSync` and always pass without a server. AC-8 targets test 4 ("OG image contains all locked content fields") which is a source-read test — it passes without a dev server. Project convention is inline hardcoded copy (no shared constants file); do not introduce a copy-constants module.

### 2026-04-12T01:21:00Z | Phase 45 Plan 01 | tags: token-rename, tailwind-v4, css-layers, build-pipeline

Tailwind v4 `@theme inline` (not bare `@theme`) makes utilities reference `--sfx-*` vars directly at runtime — essential for the consumer override pattern. The `wrap-tokens-layer.ts` parser must treat `@theme` blocks as "outside layer" by block-level detection, not line-by-line regex; brace-depth tracking handles multiline blocks correctly. `gsd-tools.cjs` is at `$HOME/.claude/plugins/marketplaces/pde/bin/pde-tools.cjs` in this project (not `pde-os/engines/gsd/bin/`). The `[data-bg-shift]` direct `--color-muted-foreground` override must be updated to `--sfx-muted-foreground` after the rename — it's not in @theme and gets silently ignored otherwise.
