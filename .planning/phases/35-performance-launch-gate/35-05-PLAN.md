...
**Maximum 2 retry iterations (brain-wins D-8, EDGE-CASES MEDIUM #4).** If after 2 fix → redeploy → re-verify cycles the production console is still not clean, the remaining non-zero findings are either (a) accepted-as-risk with Grey's written approval, logged in `v1.6-carry-overs.md`, OR (b) the cap is expanded per the 5-fix cap overflow protocol in brief §Visual-QA Wave 3. Do NOT iterate indefinitely — the retry ceiling prevents the close commit from being blocked on an infinite loop caused by a persistent third-party warning (e.g., a CSP violation from a third-party script that won't fix itself). See `wiki/analyses/v1.5-phase35-reconciliation.md` §D-8.
  </action>
  <verify>
...
**If Grey is unreachable AND any category < 100 AND the 5-fix cap is exhausted (brain-wins D-4, EDGE-CASES HIGH #2):**

The close commit CANNOT land with an unresolved Lighthouse category failure — the `Lighthouse verified...` audit string would be false. But the plan must not deadlock forever waiting for Grey. Follow this explicit BLOCK path:

1. **Do NOT land the close commit.** Phase 35 is in a BLOCKED state — not Complete, not rolled back, just paused.
2. **Update `.planning/STATE.md` frontmatter:**
   ```yaml
   status: blocked_awaiting_grey
   stopped_at: Phase 35 Lighthouse <category> at <score> — cap exhausted, awaiting Grey escalation decision (expand-cap / accept-as-risk / defer-to-v1.6)
   ```
3. **Append a marker line to `.planning/phases/35-performance-launch-gate/production-console-sweep.md`** (or create it if Task 2 also BLOCKED):
   ```
   **BLOCKED:** Lighthouse <category> = <score>, cap exhausted, waiting on Grey's expand-cap / accept-as-risk / defer-to-v1.6 decision (filed <timestamp>).
   ```
4. **The next session** (human or Claude) resumes from STATE.md and either applies Grey's decision or continues waiting. The block is surfaceable via `/pfcr` and `/lfcr` per the context refresh protocol.
5. **Do NOT silently accept the failing category.** Do NOT silently expand the cap without Grey's written statement. Silence is worse than delay — the audit trail must remain honest.

This is explicitly NOT a "pick one and proceed" path. The close commit is the load-bearing v1.5 ship marker; it must reflect reality, not convenience. See `wiki/analyses/v1.5-phase35-reconciliation.md` §D-4 for the full trap analysis.
  </action>
  <boundaries>
    <!-- EDGE-CASES HIGH #2 (brain-wins D-4): if Lighthouse fails AND cap exhausted AND Grey unreachable, the close commit MUST NOT land; phase BLOCKS with an explicit "awaiting_grey" marker in STATE.md. Silent acceptance of a failing category or silent cap expansion is forbidden. The block is explicit and recoverable; silence is neither. -->
  </boundaries>
  <verify>
  ...
  <done>launch-gate-final.json exists with valid structure. Script ran against production URL. Close-commit Lighthouse verification string is ready. VL-05 slash sibling unchanged. If Grey-unreachable + cap-exhausted + failing-category triad occurred, STATE.md reflects the blocked_awaiting_grey status and the close commit is NOT landed.</done>
</task>
