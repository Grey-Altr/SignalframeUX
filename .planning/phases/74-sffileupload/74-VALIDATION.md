...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 74-01-01 | 01 | 1 | FU-01 | drop-zone-keyboard-trap | Drop zone has `role="button"` + visible focus ring + hidden `<input type="file">` keyboard fallback | grep | `grep -c 'role="button"' components/sf/sf-file-upload.tsx` (≥ 1) AND `grep -c 'type="file"' components/sf/sf-file-upload.tsx` (≥ 1) | ✅ W0 | ✅ green |
| 74-01-02 | 01 | 1 | FU-01 | drop-handler-bypass | `onDrop` + `onDragOver` + `e.preventDefault()` on dragover prevent browser default download behavior | grep | `grep -c 'onDrop=' components/sf/sf-file-upload.tsx` (≥ 1) AND `grep -c 'onDragOver=' components/sf/sf-file-upload.tsx` (≥ 1) AND `grep -c 'preventDefault' components/sf/sf-file-upload.tsx` (≥ 2) | ✅ W0 | ✅ green |
| 74-01-03 | 01 | 1 | FU-01 | per-file-remove-missing | Each file row has `aria-label` containing "Remove" or "Delete" | grep | `grep -c 'aria-label.*[Rr]emove' components/sf/sf-file-upload.tsx` (≥ 1) | ✅ W0 | ✅ green |
| 74-01-04 | 01 | 1 | FU-02 | mime-not-validated | MIME validation against `accept` prop runs on `File.type` and/or extension | grep | `grep -cE 'accept.*split\|\.type\.startsWith\|endsWith' components/sf/sf-file-upload.tsx` (≥ 1) | ✅ W0 | ✅ green |
| 74-01-05 | 01 | 1 | FU-02 | size-not-validated | `maxSize` prop checked against `file.size` | grep | `grep -cE 'file\.size\|maxSize' components/sf/sf-file-upload.tsx` (≥ 2) | ✅ W0 | ✅ green |
| 74-01-06 | 01 | 1 | FU-02 | error-not-surfaced | `SFFileEntry.error?: string` field present in type defs | grep | `grep -cE 'error\?:\s*string' components/sf/sf-file-upload.tsx` (≥ 1) | ✅ W0 | ✅ green |
...
| 74-01-14 | 01 | 1 | FU-04 | barrel-non-export | `SFFileUpload` exported from `components/sf/index.ts` | grep | `grep -cE 'SFFileUpload' components/sf/index.ts` (≥ 1) | ✅ W0 | ✅ green |
| 74-02-01 | 02 | 2 | FU-05 | gap-papered-over | `74-VERIFICATION.md` exists and explicitly documents `dataTransfer.files` permanent gap | shell | `test -f .planning/phases/74-sffileupload/74-VERIFICATION.md && grep -c 'dataTransfer\.files' .planning/phases/74-sffileupload/74-VERIFICATION.md` (≥ 1) | ✅ W0 | ✅ green |
| 74-02-02 | 02 | 2 | FU-05 | gap-undocumented-source | Primary source citation present (Chromium bug, microsoft/playwright issue, or both) | grep | `grep -cE 'crbug\|playwright#\|microsoft/playwright\|github\.com/microsoft/playwright' .planning/phases/74-sffileupload/74-VERIFICATION.md` (≥ 1) | ✅ W0 | ✅ green |
| 74-02-03 | 02 | 2 | FU-05 | story-missing | Storybook story file exists at canonical path with DragActive (or named-equivalent) story | shell | `test -f stories/sf-file-upload.stories.tsx && grep -cE 'DragActive\|DragState\|DragOver' stories/sf-file-upload.stories.tsx` (≥ 1) | ✅ W0 | ✅ green |
| 74-02-04 | 02 | 2 | TST-03 | vacuous-green | Drop zone `role="button"` `toBeVisible()` BEFORE `axe.analyze()` runs in axe spec | grep | `grep -cE 'role="button"\|toBeVisible' tests/v1.10-phase74-sf-file-upload-axe.spec.ts` (≥ 1) | ✅ W0 | ✅ green |
| 74-02-05 | 02 | 2 | TST-03 | a11y-violation | axe spec runs with zero violations across (empty / with-files / with-progress / with-error) fixtures | playwright+axe | `pnpm exec playwright test tests/v1.10-phase74-sf-file-upload-axe.spec.ts --project=chromium` (all PASS) | ✅ W0 | ✅ green |
| 74-02-06 | 02 | 2 | TST-03 | aria-live-missing | `aria-live` region announces selection / removal / error to screen readers | grep | `grep -c 'aria-live' components/sf/sf-file-upload.tsx` (≥ 1) | ✅ W0 | ✅ green |
| 74-02-07 | 02 | 2 | FU-01 / FU-02 / FU-03 | acceptance-broken | E2E spec covers click-to-browse, multi-file, MIME reject, size reject, controlled `onChange`, progress render, image preview | playwright | `pnpm exec playwright test tests/v1.10-phase74-sf-file-upload.spec.ts --project=chromium` (all PASS) | ✅ W0 | ✅ green |
| 74-02-08 | 02 | 2 | TST-04 | drag-test-vacuous | Drag-drop visual state verified via Storybook `play()` story; Playwright spec does NOT call `page.dispatchEvent('drop', ...)` and assert green without `dataTransfer.files` round-trip | human review | Manual review of `tests/v1.10-phase74-sf-file-upload.spec.ts` AND `stories/sf-file-upload.stories.tsx` | ✅ W0 | ✅ green |
| 74-02-09 | 02 | 2 | FU-04 | bundle-regression | Homepage First Load JS ≤ 200 KB; SFFileUpload tree-shake-clean (NOT loaded on `/`) | playwright + bundle | `pnpm exec playwright test tests/v1.8-phase63-1-bundle-budget.spec.ts --project=chromium` (PASS) AND `node -e "const m=JSON.parse(require('fs').readFileSync('.next/app-build-manifest.json','utf8')); const h=JSON.stringify(m.pages['/page']\|\|[]); console.log(/sf-file-upload/.test(h)?'FAIL':'PASS')"` (PASS) | ✅ W0 | ✅ green |
| 74-02-10 | 02 | 2 | FU-01 | space-key-panel-advance | Space-key on focused drop zone opens file dialog AND does NOT advance panel (LOCKDOWN R-64-d focus guard); regression test added per plan-checker INFO 6 | playwright | `pnpm exec playwright test tests/v1.10-phase74-sf-file-upload.spec.ts --grep "Space on drop zone" --project=chromium` (PASS) | ✅ W0 | ✅ green |
...
- [x] `tests/v1.10-phase74-sf-file-upload.spec.ts` — Playwright spec covering FU-01 (click-to-browse via `setInputFiles`, file list, per-file remove, paste handler exists, R-64-d Space-key regression), FU-02 (MIME reject, size reject, multi-file mode, mixed accept/reject), FU-03 (controlled `files` + `onChange`, `progress` prop drives `SFProgress`, image preview via object URL, disabled state) — **12/12 green**
- [x] `tests/v1.10-phase74-sf-file-upload-axe.spec.ts` — axe-core spec across 4 fixture states (empty / with-files / with-progress / with-error / disabled) + structural aria-live presence, with `[role="button"][aria-label]` `toBeVisible()` vacuous-green guard before each `analyze()` — **6/6 green**
- [x] `app/dev-playground/sf-file-upload/page.tsx` — fixture page mounting 5 sections (Plan 01 ship)
- [x] `stories/sf-file-upload.stories.tsx` — 8 Storybook stories incl. **DragActive** with `play()` + `fireEvent.dragOver()` from `storybook/test` (v10 namespace; canonical for Storybook 10.3.5)
- [x] No new framework installs — Playwright, axe-core, Storybook, Chromatic all present per Phase 71/72/73 precedent
...
- [x] Feedback latency < 25s (acceptance 14.8s + axe 9.1s, both well under 25s)
